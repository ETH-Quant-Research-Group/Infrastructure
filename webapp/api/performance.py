...
from webapp import store as _store
...
async def get_fund() -> dict:
    ...
    if _store.broker_pnl_latest:
        ...
        total_realized = sum(
            float(s.get("total_realized", 0) or 0)
            for s in broker_exchange_states.values()
        )
        total_unrealized = sum(
            float(s.get("total_unrealized", 0) or 0)
            for s in broker_exchange_states.values()
        )
...
async def get_metrics() -> dict:
    ...
    aum = sum(
        float(s.get("total_equity", 0) or 0) for s in broker_exchange_states.values()
    )
    ...
    if len(bucketed) >= 3:
        returns = [bucketed[i][1] - bucketed[i - 1][1] for i in range(1, len(bucketed))]
        n = len(returns)
...
async def _fetch_funding_and_fees(
    symbols: list[str], lookback_hours: int = 720
) -> dict:
    ...
    Returns aggregated breakdown::

        {
          "per_symbol": {
            "ETHUSDT": {
                "funding": +0.94, "fees": -15.31, "net": -14.37,
                "settlements": 6, "trades": 12,
            },
            ...
          },
