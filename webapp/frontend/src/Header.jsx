...
// Deploy is deliberately not linked here — it'll live under a separate,
// access-gated path once that's built, not in the public nav.
const NAV_LINKS = [
  { label: 'Home', to: '/' },
...
// path segment (lowercase) -> nav label
const TAB_LABELS = {
    ...
    <>
      ...
      {showToggle && (
        <div className="ml-auto flex items-center gap-1">
          ...
          <InternalLink colorClass={toggleClass} />
        </div>
      )}
    </>
  )
...
export default function Header({ onToggleTheme, overlay = false, fadeHeight = 0 }) {
  ...
  const activeLabel = location.pathname === '/'
    ...
    : location.pathname.startsWith('/strategy/')
      ? 'Strategies'
      : (TAB_LABELS[location.pathname.split('/')[1]] || 'Performance')
