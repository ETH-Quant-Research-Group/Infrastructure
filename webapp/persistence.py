...
class DashboardDB:
    ...
    def insert_broker_pnl(self, record: BrokerPnLRecord) -> None:
        self._conn.execute(
            ...
            "(total_realized, total_unrealized, total, timestamp) "
            "VALUES (?, ?, ?, ?)",
            (
    ...
    def insert_strategy_pnl(self, record: PnLRecord) -> None:
        self._conn.execute(
            ...
            "(strategy_id, total_realized, total_unrealized, total, timestamp) "
            "VALUES (?, ?, ?, ?, ?)",
            (
                record["strategy_id"],
                record["total_realized"],
    ...
    def insert_order(self, record: OrderRecord) -> None:
        self._conn.execute(
            ...
            "reduce_only, exchange, placed_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (
                record["symbol"],
                record["side"],
    ...
    def insert_fill(self, record: FillRecord) -> None:
        self._conn.execute(
            "INSERT INTO fills "
            "(strategy_id, symbol, quantity, fill_price, exchange, filled_at) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (
                record["strategy_id"],
                record["symbol"],
    ...
    def load_orders(self, limit: int = 500) -> list[OrderRecord]:
        rows = self._conn.execute(
            "SELECT symbol, side, order_type, quantity, price, "
            "reduce_only, exchange, placed_at "
