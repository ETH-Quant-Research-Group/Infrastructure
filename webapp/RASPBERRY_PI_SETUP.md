...
- `workers.webapp` — FastAPI + NATS bridge + WebSocket (port 8000)
- `cloudflared` — tunnel to qrfzurich.com

The webapp connects directly to the local NATS server and relays
...
scp webapp/cloudflared-config.yml \
    carloteufel@raspberrypi:~/Infrastructure/webapp/cloudflared-config.yml
```
...
# Terminal 5 — webapp
uv run uvicorn workers.webapp:app --port 8000

# Terminal 6 — tunnel
cloudflared tunnel --config webapp/cloudflared-config.yml run my-dashboard
```
